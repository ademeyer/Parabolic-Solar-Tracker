...
int main()
{
  ...
  IGeoDateTimeService dt;
  IGeoLocationService gps;
  IGeoWeatherService weather;

  SPA_Input spa_in(dt.GetServiceData(),
                   gps.GetServiceData(),
                   weather.GetServiceData());

  // Configure Input to get local north direction
  InData in;
  in.decimalYear = dt.GetServiceData().GetDecimalYear();
  in.pos = gps.GetServiceData();
