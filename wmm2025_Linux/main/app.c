...
int main()
{
  ...
  MAGtype_Ellipsoid Ellip;
  MAGtype_Geoid Geoid;
  int ElementOption = 1, PrintOption = 2, i, epochs = 1, UncertaintyOption = 1;
  double cord_step_size = 0, altitude_step_size = 0, time_step_size = 0;
  ...
  MAGtype_CoordGeodetic minimum = {
      .phi = 51.045,
      .lambda = -114.0618,
      .HeightAboveGeoid = 1.1812};
  MAGtype_CoordGeodetic maximum = minimum;
  ...
  MAGtype_Date enddate = startdate;
    ...
    for (minimum.phi = b; minimum.phi <= maximum.phi; minimum.phi += cord_step_size) /*Latitude loop*/
    {
      ...
      for (minimum.lambda = c; minimum.lambda <= maximum.lambda; minimum.lambda += cord_step_size) /*Longitude loop*/
      {
        ...
        for (StartDate.DecimalYear = d; StartDate.DecimalYear <= EndDate.DecimalYear; StartDate.DecimalYear += time_step) /*Year loop*/
        {
          ...
          {
            if (PrintOption == 1)
              fprintf(fileout, "%5.2f %6.2f %8.4f %7.2f %10.2f", minimum.phi, minimum.lambda, minimum.HeightAboveGeoid, StartDate.DecimalYear, PrintElement);
            else
              printf("%5.2f %6.2f %8.4f %7.2f %10.2f", minimum.phi, minimum.lambda, minimum.HeightAboveGeoid, StartDate.DecimalYear, PrintElement);
          }
          else
          {
            if (PrintOption == 1)
              fprintf(fileout, "%5.2f %6.2f %8.4f %7.2f %10.2f", minimum.phi, minimum.lambda, minimum.HeightAboveEllipsoid, StartDate.DecimalYear, PrintElement);
            else
              printf("%5.2f %6.2f %8.4f %7.2f %10.2f", minimum.phi, minimum.lambda, minimum.HeightAboveEllipsoid, StartDate.DecimalYear, PrintElement);
          }
